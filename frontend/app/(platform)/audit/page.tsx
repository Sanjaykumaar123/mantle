import { getAuditEvents, getTransfers } from "@/lib/api";
...
export default async function AuditPage() {
  const [auditEvents, transfers] = await Promise.all([getAuditEvents(), getTransfers()]);
  ...
  const transferByRecordId = new Map(transfers.map((transfer) => [transfer.id.replace("TRF-", ""), transfer]));
  ...
  return (
    <div className="space-y-6">
      ...
      <InlineNotice
        ...
        description="For a complete tenant flow, audit shows asset issuance, disclosure grant, confidential transfer, and compliance passport issuance. Amounts are shown only when the connected wallet is the owner, participant, or an authorized disclosure grantee."
      />
              ...
              {auditEvents.map((item) => (
                <tr key={item.id} className="hover:bg-surface-soft/80">
                  ...
                  <td className="px-6 py-5 text-sm text-foreground">
                    {(() => {
                      const transferId = item.target.match(/Transfer #(\d+)/)?.[1] ?? "";
                      const transfer = transferByRecordId.get(transferId);
                      if (!transfer) {
                        return <span className="text-muted">N/A</span>;
                      }
                      const visible = transfer.amountVisibility.startsWith("Visible");
                      return visible ? (
                    ...
                    {(() => {
                      ...
                      return (
                        <StatusBadge tone="neutral">
                          {transfer?.amountVisibility ?? item.visibility}
                        </StatusBadge>
                      );
                    })()}
                  </td>
