...
import { decodeEventLog, isAddress, parseUnits, type Abi, type Address, type Hex } from "viem";
...
type ContractSpec = {
  name: string;
  required: "Mandatory" | "Optional" | "Optional for cRWAUSD";
  purpose: string;
  keyFunctions: string;
};
...
const CONTRACT_SPECS: ContractSpec[] = [
  ...
  },
  {
    ...
    required: "Optional for cRWAUSD",
    purpose: "Locks public RWAUSD/ERC20 at the boundary and mints confidential cRWAUSD for private internal settlement.",
...
function getBundleAddress(value: unknown, key: string, index: number, aliases: string[] = []): Address | null {
  const candidate = Array.isArray(value)
    ? value[index]
    : isPlainObject(value)
      ? [key, ...aliases].map((candidateKey) => value[candidateKey]).find((candidateValue) => typeof candidateValue === "string")
      : null;
...
function normalizeFactoryEventBundle(value: unknown): ContractBundle | null {
  if (!Array.isArray(value)) {
    const confidentialRwaToken = getBundleAddress(value, "confidentialRwaToken", 0, ["token"]);
  ...
  const confidentialRwaToken = getBundleAddress(value, "confidentialRwaToken", 1, ["token"]);
...
function normalizeFactorySettlementEventBundle(value: unknown): ContractBundle | null {
  if (!Array.isArray(value)) {
    const settlementAsset = getBundleAddress(value, "settlementAsset", 1);
    const confidentialRwaToken = getBundleAddress(value, "confidentialRwaToken", 2, ["token"]);
    ...
    const settlementVault = getBundleAddress(value, "settlementVault", 6);
...
export default function OnboardingPage() {
  ...
  const [rwaUsdMintAmount, setRwaUsdMintAmount] = useState("10000");
  const [rwaUsdMinting, setRwaUsdMinting] = useState(false);
  const [rwaUsdMintTxHash, setRwaUsdMintTxHash] = useState<Hex | null>(null);
  ...
  const deploymentMissingSettlementVault = Boolean(deployment && !deployment.contracts.settlementVault);
  ...
  async function mintDemoRwaUsd() {
    setActionError(null);
    setRwaUsdMinting(true);
    setRwaUsdMintTxHash(null);

    try {
      if (!address) {
        throw new Error("Connect wallet first.");
      }
      if (!effectiveSettlementAsset) {
        throw new Error("RWAUSD settlement asset address is not configured.");
      }
      await ensureTargetChain();

      const amountUnits = parseUnits(rwaUsdMintAmount || "0", 6);
      if (amountUnits <= BigInt(0)) {
        throw new Error("Enter a positive RWAUSD amount.");
      }

      const txHash = await writeContractAsync({
        address: effectiveSettlementAsset,
        abi: rwaUsdMintAbi,
        functionName: "mint",
      ...
      if (receipt.status !== "success") {
        throw new Error(`RWAUSD mint reverted. Tx hash: ${txHash}`);
      }
      setRwaUsdMintTxHash(txHash);
    } catch (error) {
      setActionError(
        decodeWeb3FlowError(error, "Failed to mint demo RWAUSD. This faucet only works with the demo RWAUSD contract."),
      );
    } finally {
      setRwaUsdMinting(false);
    }
  }
          ...
          >
            <div className="space-y-4">
              ...
              <InlineNotice
                ...
                description="TenantFactory uses the token owner wallet on-chain. If a settlement asset is provided, it also deploys SettlementVault so RWAUSD can be wrapped into confidential cRWAUSD."
                tone="accent"
              />

              <div className="rounded-2xl border border-border bg-surface-soft p-4">
                <label className="text-xs font-semibold uppercase tracking-[0.16em] text-muted">
                  Settlement asset for cRWAUSD wrapping
                </label>
                <input
                  ...
                  placeholder="RWAUSD ERC20 address (0x...)"
              ...
              {deploymentMissingSettlementVault ? (
                <InlineNotice
                  title="Settlement vault is not saved for this tenant"
                  description="The restored tenant bundle was deployed without SettlementVault. Keep the RWAUSD settlement asset filled, then click Deploy Settlement Bundle to create and save a new bundle with settlementVault."
                  tone="warning"
                />
              ) : null}
              ...
              <div className="rounded-2xl border border-border bg-surface-soft p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <p className="text-sm font-semibold text-foreground">Demo RWAUSD faucet</p>
                    <p className="mt-1 text-xs leading-5 text-muted">
                      Mint test RWAUSD to the connected wallet before deploying the settlement bundle. This removes the need for any external stablecoin during judge testing.
                    </p>
                    <p className="mt-2 break-all font-mono text-xs text-foreground">
                      RWAUSD contract: {effectiveSettlementAsset ?? "Not configured"}
                    </p>
                  </div>
                  <StatusBadge tone={effectiveSettlementAsset ? "success" : "warning"}>
                    {effectiveSettlementAsset ? "Faucet ready" : "Needs address"}
                  </StatusBadge>
                </div>
                <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
                  <input
                    value={rwaUsdMintAmount}
                    onChange={(event) => setRwaUsdMintAmount(event.target.value)}
                    placeholder="RWAUSD amount"
                    className={inputClassName()}
                  />
                  <Button onClick={() => void mintDemoRwaUsd()} disabled={!address || !effectiveSettlementAsset || rwaUsdMinting}>
                    {rwaUsdMinting ? "Minting RWAUSD..." : "Mint RWAUSD"}
                  </Button>
                </div>
                {rwaUsdMintTxHash ? (
                  <InlineNotice
                    title="RWAUSD minted"
                    description={`Mint transaction confirmed: ${rwaUsdMintTxHash}`}
                    tone="success"
                  />
                ) : null}
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                <Button onClick={() => void deployTenantBundle()} disabled={!address || activeAction !== null || !factoryReady}>
                  {activeAction === "deploy"
                    ...
                    : deploymentMissingSettlementVault && effectiveSettlementAsset
                      ? "Deploy Settlement Bundle"
