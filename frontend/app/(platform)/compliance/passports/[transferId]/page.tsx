...
export default async function PassportDetailPage({
  ...
  const transfer = transfers.find((item) => item.id === `TRF-${detail.transfer_record_id}`) ?? null;
  const amountVisible = Boolean(transfer?.amountVisibility.startsWith("Visible"));
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <SectionCard title="Evidence trail">
          <dl className="space-y-4 text-sm">
            <div>
              <dt className="text-xs uppercase tracking-[0.16em] text-muted">Transfer amount</dt>
              <dd className="mt-1 text-foreground">
                {transfer
                  ? amountVisible
                    ? `$${transfer.amount.toLocaleString()} (${transfer.amountVisibility})`
                    : `Encrypted (${transfer.amountVisibility})`
                  : "Transfer record unavailable"}
              </dd>
            </div>
            <div>
