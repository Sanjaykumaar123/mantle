...
describe("TenantFactory", function () {
  ...
  it("deploys a tenant settlement bundle with a vault authorized to mint confidential settlement tokens", async function () {
    ...
    expect(await rwaUsd.decimals()).to.equal(6);

    const tx = await factory.createTenantSettlementBundle(tenantOwner.address, await rwaUsd.getAddress());

    await expect(tx)
      .to.emit(factory, "TenantSettlementBundleCreated")
      .withArgs(tenantOwner.address, await rwaUsd.getAddress(), anyValue, anyValue, anyValue, anyValue, anyValue);
    ...
    expect(await vault.settlementAsset()).to.equal(await rwaUsd.getAddress());
