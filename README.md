...
2. Automatically generates `contracts/deployments/mantle-testnet.json` containing the deployed addresses.
3. Automatically updates empty keys in `contracts/.env` (e.g. `MANTLE_TESTNET_AI_SCORE_ANCHOR`).

### Native Mantle RWA Asset Support (USDY & mETH)

The platform is natively seeded with and configured to support flagship Mantle ecosystem yield assets and strategies:
* **Ondo USD Yield (USDY)** — Institutional dollar-denominated yield token.
* **Mantle Staked ETH (mETH)** — Mantle Liquid Staking Protocol yield token.
* **Mantle USDY Treasury Fund** — Multi-strategy asset management pool.
* **mETH Liquid Yield Strategy** — Automated risk-managed portfolio yield strategy.

### AIScoreAnchor – On-Chain AI Score Anchoring

Calculated risk, compliance, and confidence metrics generated by the Intelligence Center are automatically anchored on-chain to the `AIScoreAnchor` contract. The frontend detects the active chain and provides verified on-chain status badges linked directly to the **Mantle Sepolia Explorer (Mantlescan)**.

---
### Backward Compatibility

All existing Arbitrum Sepolia contracts, configuration, and workflows remain completely supported and unchanged.
